(function(e, t, n) {
	(function(e) {
		...
		"use strict";
		var i = {},
			s, o, u, a, f, l, c, h, p, d, v, m, g, y, b, w, E, S, x, T, N, C, k, L, A, O, M, _, D, P, H, B, j =
			0;
		s = function() {
			return {
				common: {
				...
				pie: {
					offset: 0,
					sliceColors: ["#3366cc", "#dc3912", "#ff9900", "#109618", "#66aa00", "#dd4477",
						"#0099c6", "#990099"
					],
					borderWidth: 0,
					borderColor: "#000",
		...
		O = ".jqstooltip { " + "position: absolute;" + "left: 0px;" + "top: 0px;" + "visibility: hidden;" +
			...
			"padding: 5px;" + "border: 1px solid white;" + "z-index: 10000;" + "}" + ".jqsfield { " +
				...
				return this.format.replace(this.fre, function() {
					...
					if (p(f)) {
						if (r.get("numberFormatter")) {
							...
							f = y(f, l, r.get("numberDigitGroupCount"), r.get(
								"numberDigitGroupSep"), r.get("numberDecimalMark"))
						}
					}
					return f
				})
		...
		E = o({
			sizeStyle: "position: static !important;" + "display: block !important;" +
				"visibility: hidden !important;" + "float: left !important;",
			init: function(t) {
		...
		r.fn.sparkline = function(t, i) {
			return this.each(function() {
				...
				u = function() {
					...
					u = s.get("width") === "auto" ? i.length * s.get("defaultPixelsPerValue") :
						s.get("width");
				...
				if (r(this).html() && !s.get("disableHiddenCheck") && r(this).is(":hidden") || !r(
		...
		r.fn.sparkline.options = o({
			init: function(e, t) {
				...
				o = s.common;
				this.tagOptionsPrefix = t.enableTagOptions && (t.tagOptionsPrefix || o
					.tagOptionsPrefix);
		...
		r.fn.sparkline._base = o({
			...
			initTarget: function() {
				...
				if (!(this.target = this.$el.simpledraw(this.width, this.height, this.options.get(
						"composite"), e))) {
					this.disabled = true
				} else {
			...
			calcHighlightColor: function(e, n) {
				...
				if (i) {
					s = /^#([0-9a-f])([0-9a-f])([0-9a-f])$/i.exec(e) ||
		...
		r.fn.sparkline.line = x = o(r.fn.sparkline._base, {
			...
			renderHighlight: function() {
				...
				if (u) {
					f = t.drawLine(r[0], this.canvasTop, r[0], this.canvasTop + this.canvasHeight,
						u);
			...
			processRangeOptions: function() {
				...
				if (e.get("chartRangeMin") !== n && (e.get("chartRangeClip") || e.get(
						"chartRangeMin") < this.miny)) {
					this.miny = e.get("chartRangeMin")
				}
				if (e.get("chartRangeMax") !== n && (e.get("chartRangeClip") || e.get(
						"chartRangeMax") > this.maxy)) {
					this.maxy = e.get("chartRangeMax")
				}
				if (e.get("chartRangeMinX") !== n && (e.get("chartRangeClipX") || e.get(
						"chartRangeMinX") < this.minx)) {
					this.minx = e.get("chartRangeMinX")
				}
				if (e.get("chartRangeMaxX") !== n && (e.get("chartRangeClipX") || e.get(
			...
			render: function() {
				...
				if (a) {
					...
					if (_ || (e.get("minSpotColor") || e.get("maxSpotColor")) && (H[0] === this
							.miny || H[0] === this.maxy)) {
						d += t.ceil(a);
						s -= t.ceil(a)
					}
					if (_ || e.get("spotColor") || e.get("minSpotColor") || e.get("maxSpotColor") &&
						(H[h] === this.miny || H[h] === this.maxy)) {
						s -= t.ceil(a)
					}
				}
				o--;
				...
				if (a && e.get("valueSpots")) {
					...
					for (B = 0; B < k; B++) {
						D = M.get(H[B]);
						if (D) {
							i.drawCircle(d + t.round((P[B] - this.minx) * (s / l)), p + t.round(o -
								o * ((H[B] - this.miny) / c)), a, n, D).append()
						}
					}
				}
				if (a && e.get("spotColor") && H[h] !== null) {
					i.drawCircle(d + t.round((P[P.length - 1] - this.minx) * (s / l)), p + t.round(
						o - o * ((H[h] - this.miny) / c)), a, n, e.get("spotColor")).append()
				}
				if (this.maxy !== this.minyorg) {
					if (a && e.get("minSpotColor")) {
						y = P[r.inArray(this.minyorg, H)];
						i.drawCircle(d + t.round((y - this.minx) * (s / l)), p + t.round(o - o * ((
								this.minyorg - this.miny) / c)), a, n, e.get("minSpotColor"))
							.append()
					}
					if (a && e.get("maxSpotColor")) {
						y = P[r.inArray(this.maxyorg, H)];
						i.drawCircle(d + t.round((y - this.minx) * (s / l)), p + t.round(o - o * ((
								this.maxyorg - this.miny) / c)), a, n, e.get("maxSpotColor"))
							.append()
					}
				}
		...
		r.fn.sparkline.bar = T = o(r.fn.sparkline._base, S, {
			...
			init: function(e, i, s, o, u) {
				...
				if (s.get("chartRangeMin") !== n && (s.get("chartRangeClip") || s.get(
						"chartRangeMin") < M)) {
					M = s.get("chartRangeMin")
				}
				if (s.get("chartRangeMax") !== n && (s.get("chartRangeClip") || s.get(
						"chartRangeMax") > _)) {
		...
		r.fn.sparkline.discrete = C = o(r.fn.sparkline._base, S, {
			...
			init: function(e, i, s, o, u) {
				...
				this.initTarget();
				if (this.target) {
					this.lineHeight = s.get("lineHeight") === "auto" ? t.round(this.canvasHeight *
						.3) : s.get("lineHeight")
				}
			},
			getRegion: function(e, n, r) {
			...
			renderRegion: function(e, n) {
				...
				g = e * f;
				d = t.round(p - p * ((v - s) / u));
				m = i.get("thresholdColor") && v < i.get("thresholdValue") ? i.get(
					"thresholdColor") : i.get("lineColor");
				if (n) {
		...
		r.fn.sparkline.bullet = k = o(r.fn.sparkline._base, {
			...
			renderPerformance: function(e) {
				...
				return this.target.drawRect(0, t.round(this.canvasHeight * .3), r - 1, t.round(this
					.canvasHeight * .4) - 1, i, i)
			},
			renderTarget: function(e) {
				var n = this.values[0],
					r = t.round(this.canvasWidth * ((n - this.min) / this.range) - this.options.get(
						"targetWidth") / 2),
		...
		r.fn.sparkline.pie = L = o(r.fn.sparkline._base, {
			...
			getCurrentRegionFields: function() {
				...
				return {
					...
					color: this.options.get("sliceColors")[e % this.options.get("sliceColors")
						.length],
					offset: e
				}
			},
		...
		r.fn.sparkline.box = A = o(r.fn.sparkline._base, {
			...
			render: function() {
				...
				if (s.get("showOutliers")) {
					...
					S = o / (l - a + 1);
					if (p < h) {
						e.drawCircle((p - a) * S + c, u / 2, s.get("spotRadius"), s.get(
							"outlierLineColor"), s.get("outlierFillColor")).append()
					}
					if (b > y) {
						e.drawCircle((b - a) * S + c, u / 2, s.get("spotRadius"), s.get(
							"outlierLineColor"), s.get("outlierFillColor")).append()
					}
				}
				e.drawRect(t.round((v - a) * S + c), t.round(u * .1), t.round((g - v) * S), t.round(
					u * .8), s.get("boxLineColor"), s.get("boxFillColor")).append();
				e.drawLine(t.round((h - a) * S + c), t.round(u / 2), t.round((v - a) * S + c), t
					.round(u / 2), s.get("lineColor")).append();
				e.drawLine(t.round((h - a) * S + c), t.round(u / 4), t.round((h - a) * S + c), t
					.round(u - u / 4), s.get("whiskerColor")).append();
				e.drawLine(t.round((y - a) * S + c), t.round(u / 2), t.round((g - a) * S + c), t
					.round(u / 2), s.get("lineColor")).append();
				e.drawLine(t.round((y - a) * S + c), t.round(u / 4), t.round((y - a) * S + c), t
					.round(u - u / 4), s.get("whiskerColor")).append();
				e.drawLine(t.round((m - a) * S + c), t.round(u * .1), t.round((m - a) * S + c), t
					.round(u * .9), s.get("medianColor")).append();
				if (s.get("target")) {
					E = t.ceil(s.get("spotRadius"));
					e.drawLine(t.round((s.get("target") - a) * S + c), t.round(u / 2 - E), t.round((
						s.get("target") - a) * S + c), t.round(u / 2 + E), s.get(
						"targetColor")).append();
					e.drawLine(t.round((s.get("target") - a) * S + c - E), t.round(u / 2), t.round((
						s.get("target") - a) * S + c + E), t.round(u / 2), s.get(
						"targetColor")).append()
				}
				e.render()
			}
		...
		P = o(D, {
			...
			_drawShape: function(e, t, r, i, s) {
				...
				if (this.targetX !== n && this.targetY !== n && o.isPointInPath(this.targetX, this
						.targetY)) {
			...
			_drawCircle: function(e, r, i, s, o, u, a) {
				...
				if (this.targetX !== n && this.targetY !== n && f.isPointInPath(this.targetX, this
						.targetY)) {
					this.currentTargetShapeId = e
				}
				if (o !== n) {
		...
		H = o(D, {
			init: function(t, n, i) {
				...
				this.canvas.width = this.pixelWidth;
				this.canvas.height = this.pixelHeight;
				s = '<v:group coordorigin="0 0" coordsize="' + this.pixelWidth + " " + this
					.pixelHeight + '"' + ' style="position:absolute;top:0;left:0;width:' + this
					.pixelWidth + "px;height=" + this.pixelHeight + 'px;"></v:group>';
			...
			_drawShape: function(e, t, r, i, s) {
				...
				u = o.splice(0, 1);
				s = s === n ? 1 : s;
				a = r === n ? ' stroked="false" ' : ' strokeWeight="' + s + 'px" strokeColor="' +
					r + '" ';
				f = i === n ? ' filled="false"' : ' fillColor="' + i + '" filled="true" ';
				l = o[0] === o[o.length - 1] ? "x " : "";
				c = '<v:shape coordorigin="0 0" coordsize="' + this.pixelWidth + " " + this
					.pixelHeight + '" ' + ' id="jqsshape' + e + '" ' + a + f +
					' style="position:absolute;left:0px;top:0px;height:' + this.pixelHeight +
					"px;width:" + this.pixelWidth + 'px;padding:0px;margin:0px;" ' + ' path="m ' +
					u + " l " + o.join(", ") + " " + l + 'e">' + " </v:shape>";
				return c
			},
			_drawCircle: function(e, t, r, i, s, o, u) {
				...
				t -= i;
				r -= i;
				a = s === n ? ' stroked="false" ' : ' strokeWeight="' + u + 'px" strokeColor="' +
					s + '" ';
				f = o === n ? ' filled="false"' : ' fillColor="' + o + '" filled="true" ';
				l = "<v:oval " + ' id="jqsshape' + e + '" ' + a + f +
					' style="position:absolute;top:' + r + "px; left:" + t + "px; width:" + i * 2 +
					"px; height:" + i * 2 + 'px"></v:oval>';
				return l
			},
			_drawPieSlice: function(e, r, i, s, o, u, a, f) {
				...
				g = '<v:shape coordorigin="0 0" coordsize="' + this.pixelWidth + " " + this
					.pixelHeight + '" ' + ' id="jqsshape' + e + '" ' + v + m +
					' style="position:absolute;left:0px;top:0px;height:' + this.pixelHeight +
					"px;width:" + this.pixelWidth + 'px;padding:0px;margin:0px;" ' + ' path="m ' +
					r + "," + i + " wa " + l.join(", ") + ' x e">' + " </v:shape>";
				return g
			},
			_drawRect: function(e, t, n, r, i, s, o) {
				return this._drawShape(e, [
